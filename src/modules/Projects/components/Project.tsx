...
import { ProjectProps } from "../consts/IProject";
import { useDeleteProjectMutation } from "../api/apiProjectSlice";
  ...
  return (
    <tr className="table_row" key={project.id}>
      <td className="table_d column1">
        <Link className="table__td-button" to={`/teplo/projects/${project.id}/rooms`}>
          <div className="table__icon" />
          {project.name}
        </Link>
      </td>
      <td className="table_d column1-1">{project.region}</td>
      ...
      <td className="table_d column4">
        <button className="table__delete" type="button" onClick={onDeleteproject} />
      </td>
    </tr>
  );
}
